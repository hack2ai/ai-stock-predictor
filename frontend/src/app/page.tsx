    ...
    <main className="min-h-screen bg-[#0b1120] text-slate-100 p-5 md:p-10">
      <div className="mx-auto max-w-7xl space-y-8">
        <header className="flex flex-col justify-between gap-6 md:flex-row md:items-center">
          <div className="flex items-center gap-4"><div className="rounded-2xl bg-blue-500/15 p-4 text-blue-400"><BrainCircuit size={30} /></div><div><h1 className="text-3xl font-black tracking-tight">AI Stock Intelligence</h1><p className="text-slate-400">Machine learning, technical analysis and transparent model metrics</p></div></div>
          <form onSubmit={handleSearch} className="relative w-full md:w-96"><Search size={18} className="absolute left-4 top-3.5 text-slate-400" /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="AAPL, TSLA, BTC-USD..." className="w-full rounded-xl border border-slate-700 bg-slate-800/70 py-3 pl-11 pr-24 outline-none focus:border-blue-500" /><button className="absolute right-1.5 top-1.5 rounded-lg bg-blue-600 px-4 py-1.5 text-sm font-semibold">Analyze</button></form>
        </header>
        <div className="rounded-2xl border border-blue-500/20 bg-gradient-to-r from-blue-500/10 to-transparent p-6"><div className="flex items-start gap-3"><Sparkles className="mt-1 text-blue-400" /><div><h2 className="font-bold">{ticker} Market Analysis</h2><p className="mt-1 text-sm text-slate-400">Historical market data, technical indicators, walk-forward validation and prediction uncertainty.</p></div></div></div>
