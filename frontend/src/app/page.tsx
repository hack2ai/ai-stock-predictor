...
export default function Home() {
  ...
  const loadAnalysis = useCallback(async (symbol: string) => {
    ...
    try {
      const response = await axios.get<Analysis>(
        `${API_BASE}/api/v1/stocks/${encodeURIComponent(symbol)}/analysis`
      );
  ...
  useEffect(() => {
    void loadAnalysis("AAPL");
  }, [loadAnalysis]);

  const handleSearch = (event: React.FormEvent) => {
    event.preventDefault();
    const symbol = query.trim().toUpperCase();
    if (symbol) void loadAnalysis(symbol);
  };

  return (
    <main className="min-h-screen bg-[#0b1120] p-5 text-slate-100 md:p-10">
      <div className="mx-auto max-w-7xl space-y-8">
        <header className="flex flex-col justify-between gap-6 md:flex-row md:items-center">
          <div className="flex items-center gap-4">
            <div className="rounded-2xl bg-blue-500/15 p-4 text-blue-400"><BrainCircuit size={30} /></div>
            <div>
              <h1 className="text-3xl font-black tracking-tight">AI Stock Intelligence</h1>
              <p className="text-slate-400">Machine learning, technical analysis and transparent model metrics</p>
            </div>
          </div>
        ...
        <div className="rounded-2xl border border-blue-500/20 bg-gradient-to-r from-blue-500/10 to-transparent p-6">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="font-bold">{ticker} Market Analysis</h2>
              <p className="mt-1 text-sm text-slate-400">Historical market data, technical indicators, walk-forward validation and prediction uncertainty.</p>
            </div>
          </div>
        </div>
