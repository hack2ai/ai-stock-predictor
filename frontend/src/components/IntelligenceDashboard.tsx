  ...
  return (
    <div className="grid gap-6 lg:grid-cols-3">
      <section className="lg:col-span-2 rounded-2xl border border-slate-700/60 bg-slate-800/40 p-6 backdrop-blur-xl">
        <div className="flex items-center gap-3 mb-6"><BrainCircuit className="text-blue-400" /><div><h2 className="text-xl font-bold">AI Market Intelligence</h2><p className="text-sm text-slate-400">Next-session statistical estimate</p></div></div>
      ...
      <section className="lg:col-span-2 rounded-2xl border border-slate-700/60 bg-slate-800/40 p-6"><h3 className="font-semibold mb-5">Historical Closing Price</h3><div className="h-80"><ResponsiveContainer width="100%" height="100%"><AreaChart data={chartData}><defs><linearGradient id="closeGradient" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#3b82f6" stopOpacity={0.35}/><stop offset="100%" stopColor="#3b82f6" stopOpacity={0}/></linearGradient></defs><CartesianGrid strokeDasharray="3 3" stroke="#334155"/><XAxis dataKey="date" tick={{fontSize:12}}/><YAxis domain={["auto", "auto"]} tick={{fontSize:12}}/><Tooltip formatter={(value: number) => `$${value.toFixed(2)}`}/><Area type="monotone" dataKey="close" stroke="#3b82f6" fill="url(#closeGradient)" strokeWidth={2}/></AreaChart></ResponsiveContainer></div></section>
