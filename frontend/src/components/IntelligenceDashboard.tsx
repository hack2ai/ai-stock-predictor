    ...
    <div className="grid gap-6 lg:grid-cols-3">
      <section className="lg:col-span-2 rounded-2xl border border-slate-700/60 bg-slate-800/40 p-6 backdrop-blur-xl">
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-400">Next-session statistical estimate</p>
          </div>
        </div>
        <div className="grid gap-4 sm:grid-cols-3">
          ...
          <MetricCard
            ...
            value={`${positive ? "+" : ""}${formatNumber(prediction.expected_change_percent)}%`}
      ...
      <section className="rounded-2xl border border-slate-700/60 bg-slate-800/40 p-6 backdrop-blur-xl">
        ...
        <div className="mt-6 flex items-center gap-3">
          {positive ? <TrendingUp className="text-emerald-400" /> : <TrendingDown className="text-red-400" />}
          <span className="text-2xl font-black">{prediction.trend}</span>
        </div>
      ...
      <section className="rounded-2xl border border-blue-500/30 bg-blue-500/5 p-6">
        ...
        <div className="mt-5 space-y-4">
          ...
          <div><p className="text-xs text-slate-500">Trading volume</p><p className="text-xl font-bold">{analysis.latest_market.volume.toLocaleString()}</p></div>
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-slate-700/60 bg-slate-800/40 p-6">
        ...
        <div className="grid grid-cols-2 gap-3 text-sm">
          {Object.entries(analysis.technical_indicators ?? {}).slice(0, 8).map(([key, value]) => (
      ...
      <section className="rounded-2xl border border-slate-700/60 bg-slate-800/40 p-6">
        ...
        <div className="space-y-3 text-sm">
          <p>MAE <span className="float-right font-bold">{formatNumber(prediction.metrics?.mae, 4)}</span></p>
          <p>RMSE <span className="float-right font-bold">{formatNumber(prediction.metrics?.rmse, 4)}</span></p>
          <p>R² <span className="float-right font-bold">{formatNumber(prediction.metrics?.r2, 4)}</span></p>
        </div>
      </section>
        ...
        {topFeatures.length > 0 ? (
          <div className="space-y-3">
            {topFeatures.map(([name, score]) => (
              <div key={name}>
                <div className="mb-1 flex justify-between text-xs"><span>{name}</span><span>{formatNumber(score * 100, 1)}%</span></div>
                <div className="h-2 rounded bg-slate-700"><div className="h-2 rounded bg-blue-500" style={{ width: `${Math.min(Math.max(score * 100 * 4, 0), 100)}%` }} /></div>
              </div>
            ))}
          </div>
        ) : <p className="text-sm text-slate-500">Feature importance is unavailable for this analysis.</p>}
      </section>
    </div>
  );
}
